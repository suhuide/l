...
check_dependencies() {
    ...
    local deps=("git" "debootstrap" "squashfs-tools" "xorriso" "grub2-common" "grub-pc-bin" "grub-efi-amd64-bin" "mtools" "syslinux" "syslinux-common")
...
# FIXED: Advanced casper initramfs with switch_root fix as fallback
...
# FIXED: Enhanced initramfs creation with proper casper integration and switch_root fix
create_initramfs() {
...
# Prepare ISO structure matching official Ubuntu ISO
prepare_iso() {
    ...
    mkdir -p iso/{boot/grub,casper,isolinux,.disk,EFI/BOOT,preseed,dists/stable/main/binary-amd64,install,pool/main,misc,tools}
    ...
    cp initrd-custom iso/casper/initrd.gz
    ...
    # Create Ubuntu-style disk info files
    mkdir -p iso/.disk
    echo "$ISO_LABEL" > iso/.disk/info
    echo "full_cd/single" > iso/.disk/cd_type
    echo "Ubuntu custom build" > iso/.disk/release_notes_url
    echo "https://ubuntu.com" > iso/.disk/ubuntu_advantage_uri
    
    # Create Ubuntu marker files
    touch iso/ubuntu
    echo "Focal Fossa" > iso/.disk/base_installable
    echo "full_cd/single" > iso/.disk/cd_type
    
    # Create install directory files (like official Ubuntu)
    mkdir -p iso/install
    echo "casper/vmlinuz" > iso/install/vmlinuz
    echo "casper/initrd.gz" > iso/install/initrd.gz
    echo "casper/filesystem.squashfs" > iso/install/filesystem.squashfs
    echo "md5sum.txt" > iso/install/md5sum
    
    # Create minimal pool structure (like official Ubuntu)
    mkdir -p iso/pool/main
    touch iso/pool/main/.empty_directory
    
    # Create tools directory for boot parameter tools
    mkdir -p iso/tools
    cat > iso/tools/parted << 'TOOLS_EOF'
#!/bin/sh
echo "Minimal parted wrapper for live CD"
TOOLS_EOF
    chmod +x iso/tools/parted
...
LABEL live-nomodeset
  ...
  KERNEL /casper/vmlinuz
  APPEND initrd=/casper/initrd.gz boot=casper quiet splash nomodeset ---

LABEL oem
  MENU LABEL OEM install (for manufacturers)
  KERNEL /casper/vmlinuz
  APPEND initrd=/casper/initrd.gz boot=casper only-ubiquity oem-config/enable=true ---
...
GRUB_EFI_EOF
    ...
    if [ -f "EFI/BOOT/bootia32.efi" ]; then
        ...
        log_info "ℹ 32-bit UEFI bootloader not created (normal on most modern systems)"
    fi
...
# FIXED: Enhanced ISO structure verification
verify_iso_structure() {
    ...
    [ ! -f "casper/initrd.gz" ] && missing_files+=("casper/initrd.gz")
    ...
    [ ! -f "EFI/BOOT/bootia32.efi" ] && warnings+=("EFI/BOOT/bootia32.efi")
    ...
    if [ ${#warnings[@]} -ne 0 ]; then
        for warning in "${warnings[@]}"; do
            if [ "$warning" = "EFI/BOOT/bootia32.efi" ]; then
                log_info "ℹ 32-bit UEFI bootloader not available (normal on modern systems)"
            else
...
generate_iso() {
    ...
    log_info "Generating bootable ISO (Ubuntu style)..."
    ...
    if [ -f "/usr/lib/syslinux/isohdpfx.bin" ]; then
        ...
        log_warn "Cannot find isohdpfx.bin, using default MBR"
        isohdpfx="/usr/lib/syslinux/mbr.bin"
    fi
    ...
    # Add EFI boot entry if 64-bit EFI bootloader exists
    if [ -f "iso/EFI/BOOT/bootx64.efi" ]; then
        xorriso_cmd="$xorriso_cmd -e EFI/BOOT/bootx64.efi -no-emul-boot"
        log_info "Including 64-bit UEFI boot support"
    fi
    
    xorriso_cmd="$xorriso_cmd -isohybrid-gpt-basdat \"$WORK_DIR/iso\""
...
main() {
    ...
    if [ "$KERNEL_ONLY" = false ] && [ "$ROOTFS_ONLY" = false ]; then
        ...
        log_info "  BIOS: qemu-system-x86_64 -cdrom $ISO_FILE -m 2048"
        log_info "  UEFI: qemu-system-x86_64 -cdrom $ISO_FILE -m 2048 -bios /usr/share/ovmf/OVMF.fd"
    fi
}
